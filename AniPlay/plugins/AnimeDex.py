...
class AnimeDex:
    ...
    def search(query):
        url = "https://gogoapi.rifatahmed196506.workers.dev/search/" + str(
    ...
    def anime(id):
        data = requests.get("https://gogoapi.rifatahmed196506.workers.dev/anime/" + id).json()[
            "results"
        ]
    ...
    def episode(id):
        data = requests.get("https://gogoapi.rifatahmed196506.workers.dev/episode/" + id).json()[
            "results"
        ]
        text = data["name"]
        surl = [
    ...
    def download(id):
        data = requests.get("https://gogoapi.rifatahmed196506.workers.dev/download/" + id).json()[
            "results"
        ]
        return data
